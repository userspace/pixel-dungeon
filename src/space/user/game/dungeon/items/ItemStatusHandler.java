...
public class ItemStatusHandler<T extends Item> {
	...
	public void save( Bundle bundle ) {
		for (int i=0; i < items.length; i++) {
			String itemName = items[i].getName().replace(".","_").replace("$","__").replace("space_user_game_dungeon_","");
	...
	private void restore( Bundle bundle, String[] allLabels, Integer[] allImages ) {
		...
		for (int i=0; i < items.length; i++) {
			...
			Class<? extends T> item = (Class<? extends T>)(items[i]);
			String itemName = item.getName().replace(".","_").replace("$","__").replace("space_user_game_dungeon_","");
