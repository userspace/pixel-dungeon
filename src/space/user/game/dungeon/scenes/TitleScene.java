...
import space.user.game.noosa.BitmapText;
...
import space.user.game.dungeon.effects.Fireball;
import space.user.game.dungeon.ui.Archs;
import space.user.game.dungeon.ui.ExitButton;
import space.user.game.dungeon.ui.PrefsButton;
...
public class TitleScene extends PixelScene {
    ...
    private static final String TXT_USERSPACE   = "MyUserSpace";
    private static final String TXT_RELOGIN   = "Relogin";
	@Override
	public void create() {
		...
		DashboardItem btnDashboard = new DashboardItem( TXT_USERSPACE, 4 ) {
			@Override
			protected void onClick() {
                Map event = new HashMap();
                event.put("location","home");
                event.put("from","titlescene");
                KeenClient.client().addEventAsync("direction", event);
                PixelDungeon.instance.startActivity(
                    new Intent(Intent.ACTION_VIEW, Uri.parse("http://my.user.space/login?token=" + PixelDungeon.instance.token))
                );
            }
		};
		add( btnDashboard );

        DashboardItem btnRelogin = new DashboardItem( TXT_RELOGIN, 5 ) {
            @Override
            protected void onClick() {
                PixelDungeon.login();
            }
		...
		if (PixelDungeon.landscape()) {
            ...
            btnDashboard    .setPos( w / 2 - DashboardItem.SIZE, y);
            btnRelogin      .setPos( w / 2 - DashboardItem.SIZE, y);
		} else {
			btnBadges       .setPos( w / 2 - btnBadges.width(), (h + height) / 2 - DashboardItem.SIZE );
			btnAbout        .setPos( w / 2, (h + height) / 2 - DashboardItem.SIZE );
			btnPlay         .setPos( w / 2 - btnPlay.width(), btnAbout.top() - DashboardItem.SIZE );
			btnHighscores   .setPos( w / 2, btnPlay.top() );
            btnDashboard    .setPos( w / 2 - btnRelogin.width(), (h + height) / 2 );
            btnRelogin      .setPos( w / 2, (h + height) / 2 );
		}
